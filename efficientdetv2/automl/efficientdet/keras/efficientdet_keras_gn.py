...
class FNode(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self.apply_gn_for_resampling = apply_gn_for_resampling
    self.separable_conv = separable_conv
    self.act_type = act_type
    self.is_training_gn = is_training_gn
    self.conv_after_downsample = conv_after_downsample
    self.strategy = strategy
    self.data_format = data_format
    self.weight_method = weight_method
    self.conv_gn_act_pattern = conv_gn_act_pattern
...
class OpAfterCombine(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    self.conv_op = conv2d_layer(
        ...
        kernel_size=(3, 3),
        padding='same',
        use_bias=not self.conv_gn_act_pattern,
        data_format=self.data_format,
        name='conv')
    ...
    self.gn = util_keras.build_group_norm(
        name='gn')

  def call(self, new_node, training):
    if not self.conv_gn_act_pattern:
      new_node = utils.activation_fn(new_node, self.act_type)
    new_node = self.conv_op(new_node)
    new_node = self.gn(new_node, training=training)
    if self.conv_gn_act_pattern:
      new_node = utils.activation_fn(new_node, self.act_type)
    return new_node
...
class ResampleFeatureMap(tf.keras.layers.Layer):
  ...
  def _maybe_apply_1x1(self, feat, training, num_channels):
    ...
    if num_channels != self.target_num_channels:
      ...
      if self.apply_gn:
        feat = self.gn(feat, training=training)
    return feat
...
class ClassNet(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      is_training_gn: True if we train the BatchNorm.
    ...
    for i in range(self.repeats):
      ...
      for level in range(self.min_level, self.max_level + 1):
        gn_per_level.append(
            util_keras.build_group_norm(
                name='class-%d-gn-%d' % (i, level),
            ))
      self.gns.append(gn_per_level)

    self.classes = self.classes_layer(
        conv2d_layer, num_classes, num_anchors, name='class-predict')

  @tf.autograph.experimental.do_not_convert
  def _conv_gn_act(self, image, i, level_id, training):
    conv_op = self.conv_ops[i]
    gn = self.gns[i][level_id]
    ...
    def _call(image):
      ...
      image = conv_op(image)
      image = gn(image, training=training)
      if self.act_type:
  ...
  def call(self, inputs, training, **kwargs):
    ...
    for level_id in range(0, self.max_level - self.min_level + 1):
      ...
      for i in range(self.repeats):
        image = self._conv_gn_act(image, i, level_id, training)
...
class SegmentationHead(tf.keras.layers.Layer):
    ...
    for level in range(max_level - min_level):
      self.con2d_ts.append(
          tf.keras.layers.Conv2DTranspose(
              ...
              data_format=data_format,
              use_bias=False))
      self.con2d_t_gns.append(
          util_keras.build_batch_norm(
              is_training_gn=is_training_gn,
              data_format=data_format,
              strategy=strategy,
              name='gn_' + str(level)))
  ...
  def call(self, feats, training):
    ...
    for con2d_t, con2d_t_gn, skip in zip(self.con2d_ts, self.con2d_t_gns,
                                         skips):
      x = con2d_t(x)
      x = con2d_t_gn(x, training)
  ...
  def __init__(self, config, name='fpn_cell'):
    ...
    for i, fnode_cfg in enumerate(self.fpn_config.nodes):
      ...
      fnode = FNode(
          ...
          config.apply_gn_for_resampling,
          config.is_training_gn,
          config.conv_after_downsample,
          config.conv_gn_act_pattern,
...
class EfficientDetNet(tf.keras.Model):
  ...
  def __init__(self,
    ...
    is_training_gn = config.is_training_gn
    ...
    # EfficientNetV1 Backbone
      ...
      override_params = {
          'batch_norm':
              utils.batch_norm_class(is_training_gn, config.strategy),
          'relu_fn':
    ...
    for level in range(6, config.max_level + 1):
      ...
      self.resample_layers.append(
          ResampleFeatureMap(
              ...
              apply_gn=config.apply_gn_for_resampling,
              is_training_gn=config.is_training_gn,
